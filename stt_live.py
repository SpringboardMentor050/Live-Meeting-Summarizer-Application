...
def main():
    ...
    if reference_file.exists():
        ...
        if wer_reference is None:
            print("\n WER(reference): skipped because reference text is empty.")
        else:
            print(f"\n WER (live_audio.wav vs reference.txt): {wer_reference:.4f}")
    else:
        print("\n reference.txt not found (WER vs reference skipped).")
    ...
    if recorded_transcript_file.exists():
        ...
        if wer_recorded is None:
            print(" WER(recorded): skipped because recorded transcript is empty.")
        else:
            print(f" WER (live_transcript vs recorded_transcript): {wer_recorded:.4f}")
    else:
        print(" outputs/recorded_transcript.txt not found (WER vs recorded skipped).")
