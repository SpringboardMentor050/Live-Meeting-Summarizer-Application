...
class MeetingSummarizerPipeline:
    ...
    def start_session(self, callback: Optional[Callable[[str], None]] = None) -> None:
        from scripts.audio_utils import AudioRecorder

        self.status_messages = []
        self._push_status("Recording", callback)
        self.recorder = AudioRecorder(stt_engine=self.stt, progress_callback=lambda msg: self._push_status(msg, callback))
        try:
            self.recorder.start()
        except Exception:
            self.recorder = None
    ...
    def _validate_microphone_audio(self, audio_path: str | None) -> None:
        if not audio_path:
            raise RuntimeError("No microphone recording was captured for this session.")
