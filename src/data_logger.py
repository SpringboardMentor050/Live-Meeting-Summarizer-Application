...
class SessionLogger:
    ...
    def delete_session(self, filename: str) -> bool:
        """Delete a session by its JSON filename. Returns True on success."""
        fp = self.output_dir / filename
        if not fp.exists() or not fp.name.startswith("session_"):
            return False
        fp.unlink()
        # Also try to remove matching parquet
        parquet_fp = fp.with_suffix(".parquet")
        if parquet_fp.exists():
            parquet_fp.unlink()
        logger.info("Session deleted → %s", fp)
        return True
